...
<table align="center" width="640" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <td align="left" valign="top">
            <?php
            //查询当前分类信息
            $sql = "select * from ".$prename."account_class where classid='$_GET[classid]' and ufid='$_SESSION[uid]'";
            $query = mysqli_query($conn,$sql);
            $cuclass = mysqli_fetch_array($query);
            //执行操作--修改分类名称
            if ($_GET['Submit']) {
                $sql = "update ".$prename."account_class set classname= '$_GET[classname2]' where classid='$_GET[classid]' and ufid='$_SESSION[uid]'";
                $query = mysqli_query($conn,$sql);
                if ($query) {
            ...
            //执行操作--转移分类
            if ($_GET['Submit2']) {
                $sql = "select * from ".$prename."account where acclassid= '$_GET[classid]' and acuserid='$_SESSION[uid]'";
                $query = mysqli_query($conn,$sql);
                while ($row = mysqli_fetch_array($query)) {
                    $sql = "update ".$prename."account set acclassid= '$_GET[toclassid]' where acid= '$row[acid]' and acuserid='$_SESSION[uid]'";
            ...
            if ($_GET['Submit3']) {
                ...
                if ($row = mysqli_fetch_array($query)) {
                    ...
                    $sql = "delete from ".$prename."account_class where classid=".$_GET['classid'];
                        ...
                        <td><form id="form2" name="form2" method="get" action="">
                            ...
                            <label>转移到
                                <select name="toclassid" id="toclassid">
                                    <?php
                                    $sql = "select * from ".$prename."account_class where classtype=$cuclass[classtype] and ufid='$_SESSION[uid]'";
                                    $query = mysqli_query($conn,$sql);
                                    while ($row = mysqli_fetch_array($query)) {
