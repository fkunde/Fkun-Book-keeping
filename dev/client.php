            ...
            if ($_GET['Submit']) {
                if ($_SESSION[uuid] != "") {
                    ...
                    foreach ($path as $key => $value) {
                        ...
                        $query = mysqli_query($conn,"insert into ".$prename."account (acmoney, acclassid, actime, acremark,zhifu,acuserid) values('$value','".$path1[$key]."','$time100','".$path3[$key]."','".$path4[$key]."','$_SESSION[uuid]')");


                        //$sql="insert into ".$prename."account (acmoney, acclassid, actime, acremark,zhifu,acuserid) values('$path','$path1','$path2','$path3','$path4','$_SESSION[uuid]')";
                        //$query=mysqli_query($conn,$sql);

                    }
                    // $sql="insert into ".$prename."account (acmoney, acclassid, actime, acremark,acuserid,zhifu) values('$_GET[money]', '$_GET[classid]', '$time100', '$_GET[remark]', '$_SESSION[uuid]', '$_GET[zhifu]')";
                    // $query=mysqli_query($conn,$sql);
                    if ($query) {
                    ...
                    $(AddButton).click(function (e)  //on add input button click
                        {
                            if (x <= MaxInputs) //max input box allowed
                            {
                                ...
                                $(InputsWrapper).append('<div><input name="zhifu[]" id="zhifu" type="text" value="1" style="display:none;"/><font color="MediumSeaGreen">金额：</font><input type="text" name="money[]" id="money" value="" style="width:70px;"/><font color="MediumSeaGreen"> 分类：</font><select name="classid[]" id="classid" style="height:26px;"><?php
                                    $sql = "select * from ".$prename."account_class where classtype=1 and ufid=$_SESSION[uuid]";
                                    $query = mysqli_query($conn,$sql);
                    ...
                    $(AddButton1).click(function (e)  //on add input button click
                        {
                            if (y <= MaxInputs1) //max input box allowed
                            {
                                ...
                                $(InputsWrapper1).append('<div><input name="zhifu[]" id="zhifu" type="text" value="2" style="display:none;"/><font color="red">金额：</font><input type="text" name="money[]" id="money" value="" style="width:70px;"/><font color="red"> 分类：</font><select name="classid[]" id="classid" style="height:26px;"><?php
                                    $sql = "select * from ".$prename."account_class where classtype=2 and ufid=$_SESSION[uuid]";
                                    $query = mysqli_query($conn,$sql);
                                    while ($acclass = mysqli_fetch_array($query)) {
                                        echo "<option value=$acclass[classid]>$acclass[classname]</option>";
                                    }
...
//查询本页显示的数据
$query_sql = "SELECT * FROM ".$prename."account where acuserid='$_SESSION[uuid]' ORDER BY actime DESC LIMIT  $offset , $pagesize";
...
$sql = "select * from ".$prename."account_class where classid=$row[acclassid] and ufid='$_SESSION[uuid]'";
...
//分页代码
//计算总数
$count_result = mysqli_query($conn,"SELECT count(*) as count FROM ".$prename."account where acuserid='$_SESSION[uuid]'");
...
$sql = "select * from ".$prename."account where acuserid='$_SESSION[uuid]' ORDER BY actime ASC";
