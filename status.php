...
<body>
    <div style="min-width:1200px;height:100vh;position:relative;">
        <table id="excel" style="float:left;width:9%;" class='table table-striped table-bordered'>
            <tr>
                <th bgcolor='#EBEBEB'>
                    <form id="form" name="form1" method="post" style="height:20px" action="">
                        <select name="year" id="year" onchange="window.location.href='status.php?year='+this.value;options[selectedIndex].value;onchange=save()">
                            <?php
        ...
        for ($monthnum = 1; $monthnum <= 12; $monthnum++) {
            ...
            $$npd = round($$nf / $$days, 2);
            $$mpd = round($$yf / $$days, 2);
            $thismonthend = date("Y-m-31");
            if ($$monthend > $thismonthend) {
                $$gsa = "-";
                $$gse = "-";
                $$gs = "-";
                $$yz = "-";
            ...
            echo '<table id="excel" style="float:left;width:7%" class="table table-striped table-bordered">';
        ...
        <?php
                                ...
                                if ($firstmonthout == $firstmonth) {
                                    ...
                                    $firstdate = date('Y-m', $firsttime);
                                    $now = date('Y-m');
                                    $sql = "select " . $prename . "month.mon as month, sum(case when " . $prename . "account.acamount is null then 0 else " . $prename . "account.acamount end) as sum ,ac1 from " . $prename . "month left join " . $prename . "account on date_format(FROM_UNIXTIME(" . $prename . "account.actime),'%Y-%m') = " . $prename . "month.mon and acuserid='$_SESSION[uid]' where  " . $prename . "account.ac1 = 2  and acuserid='$_SESSION[uid]' and " . $prename . "account.ac0 <>3 and " . $prename . "month.mon>='" . $firstdate . "' and " . $prename . "month.mon<='" . $now . "' group by month";
                                    $query = mysqli_query($conn, $sql);
        ...
        <div id="main" style="float:left;width:450px;height:450px;background-color:#fff;border: 1px solid;top:20px;">
            <script type="text/javascript">
        ...
        <div id="main2" style="float:left;width:450px;height:450px;background-color:#fff;border: 1px solid;top:20px;">
            <script type="text/javascript">
                ...
                option = {
                    ...
                    series: [{
                        ...
                        data: [
                            ...
                            $sql = "select sum(acamount) as total ," . $prename . "category.categoryname from " . $prename . "account left join " . $prename . "category on " . $prename . "account.accategory =" . $prename . "category.categoryid left join " . $prename . "account_class on " . $prename . "account.acclassid =" . $prename . "account_class.classid where " . $prename . "account.ac1 =1 and " . $prename . "account.ac0 <>3 and acuserid='$_SESSION[uid]' group by " . $prename . "account.accategory";
                            $s2 = array();
                            $query = mysqli_query($conn, $sql);
                            while ($accategory = mysqli_fetch_array($query)) {
